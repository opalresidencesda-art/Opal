...
export function OpalAIAssistant() {
  ...
  return <div className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom)+1rem)] z-40 flex flex-col items-end gap-3 sm:inset-x-auto sm:bottom-6 sm:right-6">
    <AnimatePresence initial={false}>{open ? <motion.aside id="opal-ai-assistant" role="dialog" aria-modal="false" aria-label="Asisten OPAL" initial={reduceMotion ? false : { opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={reduceMotion ? undefined : { opacity: 0, y: 10 }} className="flex h-[min(720px,calc(100dvh-5rem))] min-h-0 w-full flex-col overflow-hidden rounded-[1.25rem] border border-line bg-surface-raised text-ink shadow-[0_24px_70px_rgba(3,25,21,0.22)] sm:w-[25rem]">
