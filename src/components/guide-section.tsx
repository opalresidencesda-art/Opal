...
function SectionVisual({ slug }: { slug: string }) {
  ...
  if (slug === "stiker-kendaraan") {
    return (
      <><div className="mt-8 grid gap-0 border-y border-line sm:grid-cols-2 sm:divide-x sm:divide-line">
        <div className="py-5 sm:pr-7"><Car size={30} weight="fill" className="text-brand" aria-hidden="true" /><p className="mt-4 font-bold text-ink">Stiker mobil</p><p className="mt-1 text-sm leading-6 text-ink-muted">Dipasang di kaca depan dari bagian dalam. Dari arah depan mobil, posisinya berada di sisi kanan atas atau bawah; dari posisi pengemudi, sisi kiri.</p></div>
        <div className="border-t border-line py-5 sm:border-t-0 sm:pl-7"><Motorcycle size={30} weight="fill" className="text-brand" aria-hidden="true" /><p className="mt-4 font-bold text-ink">Stiker motor</p><p className="mt-1 text-sm leading-6 text-ink-muted">Dipasang di bagian depan motor dan diusahakan tidak menutupi pelat nomor.</p></div>
  ...
  if (slug === "renovasi") {
    return <aside className="mt-8 flex gap-4 border-l-2 border-brand bg-surface-subtle px-5 py-5"><Hammer size={29} weight="fill" className="shrink-0 text-brand" aria-hidden="true" /><div><p className="font-bold text-ink">Jam kerja renovasi</p><p className="mt-1 text-sm leading-6 text-ink-muted">Pekerjaan tukang berlangsung pukul 07.00 hingga 17.00 WIB. Pekerjaan tanpa suara bising seperti pengecatan menjadi pengecualian.</p></div></aside>;
  }
  if (slug === "sampah-rumah-tangga") {
    return <aside className="mt-8 flex items-start gap-5 border-y border-line py-6"><span className="grid size-14 shrink-0 place-items-center rounded-[17px] bg-brand text-ink-inverse"><Trash size={29} weight="fill" aria-hidden="true" /></span><div><p className="text-3xl font-extrabold tracking-[-0.055em] text-ink">240 liter</p><p className="mt-1 max-w-md text-sm leading-6 text-ink-muted">Acuan kapasitas tempat sampah pengganti agar sampah tertampung dengan tertutup dan aman.</p></div></aside>;
  }
  return null;
}

export function GuideSectionArticle({ section, fees, index }: { section: GuideSection; fees: FeeSchedule[]; index: number }) {
  return (
    <Reveal>
      <article id={section.slug} className="scroll-mt-28 border-t border-line py-12 first:border-t-0 first:pt-0 sm:py-16">
        <div className="flex items-start gap-4">
          <span className="mt-1 text-sm font-bold tabular-nums text-brand">0{index}</span>
          <div>
            <div className="flex items-center gap-3"><CheckCircle className="shrink-0 text-brand" size={22} weight="fill" aria-hidden="true" /><h2 className="text-2xl font-extrabold tracking-[-0.055em] text-ink sm:text-3xl">{section.title}</h2></div>
