...
function PropertyImageField({ property }: { property: PropertyMapSummary }) {
  ...
  return <fieldset className="grid gap-3 rounded-2xl border border-[#c8d7d0] bg-[#f4f8f6] p-3 sm:p-4"><legend className="px-1 text-xs font-extrabold text-ink">Gambar rumah</legend><input type="checkbox" name="removeImage" checked={removeImage} readOnly className="sr-only" />{displayedImage ? <div className="relative aspect-[16/10] overflow-hidden rounded-xl bg-[#dce7e1]"><Image src={displayedImage} alt={localPreview ? "Pratinjau gambar rumah baru" : `Foto rumah ${property.unitCode}`} fill unoptimized sizes="(max-width: 640px) 100vw, 30rem" className="object-cover" /></div> : <div className="grid min-h-32 place-items-center rounded-xl border border-dashed border-[#bfd2c9] bg-white px-4 text-center text-sm font-bold text-ink-muted">Pilih gambar untuk menampilkan pratinjau.</div>}<div className="flex flex-col gap-2 sm:flex-row"><label className="inline-flex min-h-11 cursor-pointer items-center justify-center rounded-xl bg-action px-4 text-center text-sm font-extrabold text-on-action"><span>{displayedImage ? "Ganti gambar" : "Pilih gambar"}</span><input ref={inputRef} name="image" type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" className="sr-only" onChange={(event) => { const file = event.currentTarget.files?.[0]; setError(""); if (!file) { clearFile(); return; } if (file.size < 1 || file.size > 5 * 1024 * 1024 || !["image/jpeg", "image/png", "image/webp"].includes(file.type)) { event.currentTarget.value = ""; setLocalPreview(null); setError("Gambar harus JPG, PNG, atau WEBP maksimal 5 MB."); return; } setRemoveImage(false); setLocalPreview(URL.createObjectURL(file)); }} /></label>{localPreview ? <button type="button" onClick={() => { clearFile(); setRemoveImage(false); }} className="min-h-11 rounded-xl border border-[#bfd2c9] px-4 text-sm font-extrabold text-ink">Batalkan penggantian</button> : property.imagePath ? <button type="button" onClick={() => { clearFile(); setRemoveImage((value) => !value); }} className="min-h-11 rounded-xl border border-[#dfb8b8] px-4 text-sm font-extrabold text-danger">{removeImage ? "Urungkan hapus" : "Hapus gambar"}</button> : null}</div><p className="text-xs font-semibold leading-5 text-ink-muted">JPG, PNG, atau WEBP. Maksimal 5 MB.</p>{error ? <p role="alert" className="text-sm font-bold text-danger">{error}</p> : null}</fieldset>;
}

function PropertyForm({ property, isPending, onSubmit }: { property: PropertyMapSummary; isPending: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { const profile = property.profile; return <form onSubmit={onSubmit} className="grid gap-4"><div className="grid grid-cols-2 gap-3"><Field label="Gang"><select name="gang" defaultValue={property.gang}><option value="1">Gang 1</option><option value="2">Gang 2</option><option value="3">Gang 3</option><option value="5">Gang 5</option></select></Field><Field label="Nomor rumah"><input name="houseNumber" required defaultValue={property.houseNumber} inputMode="numeric" /></Field></div><Field label="Status hunian"><select name="occupancyStatus" defaultValue={property.occupancyStatus ?? ""}><option value="">Belum disahkan</option><option value="self">Dihuni sendiri</option><option value="relative">Dihuni kerabat</option><option value="tenant">Dihuni penyewa</option><option value="vacant_rent">Kosong, disewakan</option><option value="vacant_sale">Kosong, dijual</option></select></Field><PropertyImageField property={property} /><Field label="Penanggung jawab"><input name="responsibleName" defaultValue={profile?.responsibleName ?? ""} /></Field><Field label="Alamat penanggung jawab"><textarea name="responsibleAddress" defaultValue={profile?.responsibleAddress ?? ""} /></Field><div className="grid grid-cols-2 gap-3"><Field label="WhatsApp"><input name="whatsapp" defaultValue={profile?.whatsapp ?? ""} inputMode="tel" /></Field><Field label="Email"><input name="contactEmail" type="email" defaultValue={profile?.contactEmail ?? ""} /></Field></div><Field label="Kepala keluarga"><input name="headOfHouseholdName" defaultValue={profile?.headOfHouseholdName ?? ""} /></Field><div className="grid grid-cols-2 gap-3"><Field label="Pekerjaan"><select name="headOfHouseholdOccupation" defaultValue={profile?.headOfHouseholdOccupation ?? ""}><option value="">Pilih</option><option value="employee">Pegawai</option><option value="entrepreneur">Wiraswasta</option><option value="student">Pelajar</option></select></Field><Field label="Jumlah penghuni"><input name="occupantsCount" type="number" min="1" max="30" defaultValue={profile?.occupantsCount ?? ""} /></Field></div><button disabled={isPending} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-action px-4 text-sm font-extrabold text-on-action disabled:opacity-60">{isPending ? <SpinnerGap size={18} className="animate-spin" /> : <CheckCircle size={18} weight="bold" />}{isPending ? "Menyimpan…" : "Simpan data rumah"}</button></form>; }
