  ...
  return <>
    <section className="overflow-hidden bg-action text-ink-inverse">
      <div className="mx-auto grid max-w-[1440px] lg:min-h-[460px] lg:grid-cols-[0.88fr_1.12fr]">
        <div className="flex items-center px-5 py-14 sm:px-8 sm:py-16 lg:px-10 lg:py-14">
          <div className="max-w-xl"><p className="text-xs font-extrabold tracking-[0.16em] text-brand-soft">LAYANAN WARGA</p><h1 className="public-display mt-5 text-4xl font-bold leading-[0.96] sm:text-5xl lg:text-6xl">Pilih kebutuhan. Selesaikan dengan jelas.</h1><p className="mt-6 max-w-lg text-base leading-7 text-ink-inverse/78 sm:text-lg">Satu tempat untuk proses resmi dan informasi lingkungan, dari pengisian data sampai tindak lanjut RT.</p></div>
        </div>
        <div className="relative min-h-[280px] overflow-hidden lg:min-h-0"><Image src="/images/opal-arrival-evening.png" alt="Akses masuk OPAL Residence pada malam hari" fill priority sizes="(max-width: 1024px) 100vw, 59vw" className="object-cover object-center" /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(11,48,41,0.45),transparent_48%)]" aria-hidden="true" /></div>
      </div>
    </section>
    ...
    <section className="mx-auto max-w-[1440px] px-5 py-12 sm:px-8 lg:px-10 lg:py-16">
      <div className="grid gap-8 border-b border-line pb-12 lg:grid-cols-[0.38fr_1fr] lg:gap-16 lg:pb-16">
        <div><p className="public-kicker">Mulai di sini</p><h2 className="public-display mt-4 text-3xl font-bold leading-[1.02] text-ink">Data rumah yang terlindungi.</h2></div>
        <Link href="/pendataan-warga" className="group relative overflow-hidden bg-brand px-6 py-8 text-on-brand transition hover:bg-brand-deep sm:px-9 sm:py-10"><UsersThree size={31} weight="fill" className="text-brand-soft" aria-hidden="true" /><h2 className="public-display mt-12 max-w-xl text-3xl font-bold leading-[1.02] sm:text-4xl">Pendataan warga</h2><p className="mt-4 max-w-xl text-base leading-7 text-on-brand/80">Isi satu kali untuk satu rumah. KTP dan KK disimpan secara privat dan hanya diperiksa pengurus yang berwenang.</p><span className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-on-brand">Isi pendataan <ArrowRight size={18} weight="bold" aria-hidden="true" /></span></Link>
      </div>
