import { Bell, CheckCircle, FileLock, GearSix, HouseLine, Receipt, SignOut, Stamp, UsersThree, Wrench } from "@phosphor-icons/react/dist/ssr";
import Image from "next/image";
import Link from "next/link";
import { redirect } from "next/navigation";
import { addAdminUser, deleteAnnouncement, issueServiceRequest, prepareMonthlyContributions, removeAdminUser, reviewResidentSubmission, reviewServiceRequest, saveAnnouncement, saveDocumentSettings, saveFeeSchedule, saveFloorPlanAsset, saveGuideSection, saveHomeSpec, savePropertyContribution, saveResource, saveStaffProfile, signOut } from "@/app/admin/actions";
import { MarkdownEditor } from "@/components/markdown-editor";
import { AdminPropertyDirectory } from "@/components/admin-property-directory";
import { AdminCashTransactionEditor } from "@/components/admin-cash-transaction-editor";
...
import { jakartaPeriod } from "@/lib/monthly-dues";
...
type AnnouncementRow = { id: string; title: string; body: string; published_at: string; pinned: boolean; published: boolean; image_path: string | null; image_alt: string };
...
function currentJakartaDate() {
  const values = new Intl.DateTimeFormat("en-US", { year: "numeric", month: "2-digit", day: "2-digit", timeZone: "Asia/Jakarta" }).formatToParts(new Date());
  const year = values.find((value) => value.type === "year")?.value;
  const month = values.find((value) => value.type === "month")?.value;
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [feesResult, announcementsResult, resourcesResult, sectionsResult, settingsResult, submissionsResult, requestsResult, propertiesResult, cashResult, contributionsResult, currentPeriodContributionsResult, activitiesResult, issuedDocumentsResult, adminUsersResult, pendingSubmissionsCountResult, pendingRequestsCountResult, issuableRequestsCountResult, activePropertyLinksCountResult, pendingContributionsCountResult, staffResult, specsResult, plansResult] = await Promise.all([
    ...
    getAllCashTransactions(supabase),
  ...
  const cashTotals = cashTransactions.reduce((totals, transaction) => ({
    income: totals.income + (transaction.direction === "income" ? transaction.amount_rupiah : 0),
    expense: totals.expense + (transaction.direction === "expense" ? transaction.amount_rupiah : 0),
  }), { income: 0, expense: 0 });
  const cashBalance = cashTotals.income - cashTotals.expense;
  ...
  return (
    <div className="admin-shell mx-auto max-w-[1440px] px-5 pb-16 pt-7 sm:px-8 sm:pt-10 lg:px-10 lg:pb-24 lg:pt-14">
      <header className="border-b border-line pb-8 lg:pb-10">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-3 text-xs font-extrabold uppercase tracking-[0.14em] text-brand-deep"><span>OPAL / Admin</span><span className="inline-flex min-h-7 items-center gap-1.5 rounded-full bg-brand-soft px-2.5 text-[0.65rem] tracking-[0.08em] text-brand-deep"><span className="size-1.5 rounded-full bg-brand" aria-hidden="true" />Ruang kerja aktif</span></div>
            <h1 className="public-display mt-5 max-w-3xl text-[2.5rem] font-bold leading-[0.96] text-ink sm:text-[3.7rem]">Pusat kendali warga.</h1>
            <p className="mt-5 max-w-2xl text-sm leading-7 text-ink-muted sm:text-base">Antrean, Kas, surat, rumah, dan konten publik tersusun dalam satu ruang kerja. Mulai dari hal yang membutuhkan tindakan hari ini.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2 lg:justify-end"><span className="mr-1 max-w-[15rem] truncate text-xs font-bold text-ink-muted" title={context.email}>{context.email}</span><Link href="/" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-line bg-surface-raised px-4 text-sm font-extrabold text-ink transition hover:border-brand hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-4">Lihat portal warga</Link><AdminActivityNotifications activities={activities} /><form action={signOut}><button type="submit" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-action px-4 text-sm font-extrabold text-on-action transition hover:bg-brand hover:text-on-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-4"><SignOut size={17} weight="bold" aria-hidden="true" /> Keluar</button></form></div>
        </div>
      </header>
      ...
      <Link href="/admin/peta-rumah" className="group mt-7 grid gap-5 border-y border-line bg-surface-raised px-5 py-6 transition-colors hover:border-brand sm:grid-cols-[minmax(0,1fr)_repeat(4,auto)] sm:items-end sm:gap-8 sm:px-7"><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-brand-deep">OPAL Atlas</p><h2 className="mt-2 text-xl font-extrabold tracking-[-0.05em] text-ink">Peta rumah operasional</h2><p className="mt-2 max-w-xl text-sm leading-6 text-ink-muted">Telusuri denah, fokus pada satu rumah, dan perbarui profil tanpa meninggalkan konteks lingkungan.</p></div><AtlasMetric label="Rumah" value={properties.length} /><AtlasMetric label="Perlu review" value={properties.filter((property) => property.resident_submissions.some((submission) => isPending(submission.status))).length} /><AtlasMetric label="Iuran tertunda" value={pendingContributionCount} /><span className="inline-flex min-h-11 items-center justify-center rounded-xl bg-action px-4 text-sm font-extrabold text-on-action transition group-hover:bg-brand group-hover:text-on-brand">Buka peta</span></Link>
      ...
      <div className="mt-10 lg:grid lg:grid-cols-[15rem_minmax(0,1fr)] lg:gap-12">
        ...
        <AdminDisclosure id="kas" icon="cash" title="Kas OPAL" description="Ruang kerja harian bendahara. Catat setiap uang masuk atau keluar, koreksi dengan riwayat, dan kirim ringkasan yang bisa dibaca warga." defaultOpen>
          <CashSection cashBalance={cashBalance} income={cashTotals.income} expense={cashTotals.expense} transactionCount={cashTransactions.length} lastUpdated={lastCashTransaction} transactions={cashTransactions} />
        </AdminDisclosure>
...
function SectionHeading({ icon, title, description }: { icon: AdminSectionIcon; title: string; description: string }) {
  ...
  return <div className="flex items-start gap-4"><span className="grid size-10 shrink-0 place-items-center border-l-2 border-brand bg-brand-soft text-brand-deep"><Icon size={21} weight="fill" aria-hidden="true" /></span><div><h2 className="text-xl font-extrabold tracking-[-0.05em] text-ink sm:text-2xl">{title}</h2><p className="mt-1.5 text-sm leading-6 text-ink-muted">{description}</p></div></div>;
}
...
function AdminOverview({ pendingSubmissions, pendingRequests, pendingContributions, issuableRequests, activePropertyLinks, documentsReady, cashBalance, cashIncome, cashExpense, cashTransactionCount, lastCashTransaction }: { pendingSubmissions: number; pendingRequests: number; pendingContributions: number; issuableRequests: number; activePropertyLinks: number; documentsReady: boolean; cashBalance: number; cashIncome: number; cashExpense: number; cashTransactionCount: number; lastCashTransaction: string | null }) {
  const pendingTotal = pendingSubmissions + pendingRequests + pendingContributions;
  return <section aria-labelledby="admin-overview" className="mt-8 overflow-hidden border border-line bg-surface-raised shadow-[0_20px_60px_rgba(5,45,39,0.08)]"><h2 id="admin-overview" className="sr-only">Ringkasan kerja RT</h2><div className="grid lg:grid-cols-[1.1fr_1fr]"><a href="#kas" className="group border-b border-line bg-action p-6 text-on-action transition-colors hover:bg-brand sm:p-8 lg:border-b-0 lg:border-r lg:p-10"><div className="flex items-start justify-between gap-5"><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-brand-highlight">Saldo Kas tercatat</p><p className="mt-4 text-4xl font-extrabold tracking-[-0.08em] sm:text-5xl">{formatRupiah(cashBalance)}</p></div><span className="grid size-11 place-items-center border border-white/25 bg-white/10 text-brand-highlight"><Receipt size={22} weight="fill" aria-hidden="true" /></span></div><div className="mt-7 grid grid-cols-2 gap-5 border-t border-white/20 pt-5"><div><p className="text-xs font-bold text-on-action/70">Pemasukan</p><p className="mt-1 text-lg font-extrabold text-brand-highlight">{formatRupiah(cashIncome)}</p></div><div><p className="text-xs font-bold text-on-action/70">Pengeluaran</p><p className="mt-1 text-lg font-extrabold text-danger">{formatRupiah(cashExpense)}</p></div></div><p className="mt-5 text-sm leading-6 text-on-action/70">{cashTransactionCount} transaksi tersimpan{lastCashTransaction ? `, terakhir ${formatAdminDate(lastCashTransaction)}.` : "."}</p><span className="mt-5 inline-flex text-sm font-extrabold text-brand-highlight group-hover:text-on-action">Buka ruang kerja Kas →</span></a><div className="grid sm:grid-cols-2"><a href="#antrean" className="group border-b border-line p-6 transition-colors hover:bg-surface-subtle sm:p-7 sm:border-r"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">Tindakan hari ini</p><p className="mt-3 text-4xl font-extrabold tracking-[-0.08em] text-ink">{pendingTotal}</p><p className="mt-2 text-sm leading-6 text-ink-muted">{pendingSubmissions} pendataan, {pendingRequests} surat, {pendingContributions} iuran perlu dicek.</p><span className="mt-4 inline-flex text-sm font-extrabold text-brand-deep group-hover:text-brand">Buka antrean →</span></a><a href="#surat" className="group border-b border-line p-6 transition-colors hover:bg-surface-subtle sm:p-7"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">Surat siap terbit</p><p className="mt-3 text-4xl font-extrabold tracking-[-0.08em] text-ink">{issuableRequests}</p><p className="mt-2 text-sm leading-6 text-ink-muted">{issuableRequests ? "Sudah disetujui dan menunggu PDF." : "Belum ada yang menunggu penerbitan."}</p><span className="mt-4 inline-flex text-sm font-extrabold text-brand-deep group-hover:text-brand">Kelola surat →</span></a><a href="#surat" className="group border-b border-line p-6 transition-colors hover:bg-surface-subtle sm:p-7 sm:border-r"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">Penerbitan surat</p><p className="mt-3 text-lg font-extrabold tracking-[-0.04em] text-ink">{documentsReady ? "Siap digunakan" : "Perlu dilengkapi"}</p><p className="mt-2 text-sm leading-6 text-ink-muted">{documentsReady ? "Identitas RT lengkap." : "Lengkapi identitas penerbit."}</p></a><a href="#rumah" className="group p-6 transition-colors hover:bg-surface-subtle sm:p-7"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">Akses rumah aktif</p><p className="mt-3 text-4xl font-extrabold tracking-[-0.08em] text-ink">{activePropertyLinks}</p><p className="mt-2 text-sm leading-6 text-ink-muted">Tautan privat yang masih dapat dipakai warga.</p></a></div></div></section>;
}
...
function AdminNavigation() {
  ...
  return <nav aria-label="Bagian admin" className="-mx-5 flex gap-6 overflow-x-auto border-y border-line bg-surface px-5 py-3 sm:mx-0 sm:px-0 lg:sticky lg:top-24 lg:mx-0 lg:h-fit lg:flex-col lg:gap-6 lg:overflow-visible lg:border-0 lg:bg-transparent lg:px-0 lg:py-0"><p className="sr-only lg:not-sr-only lg:px-1 lg:text-xs lg:font-extrabold lg:uppercase lg:tracking-[0.14em] lg:text-ink-faint">Navigasi ruang kerja</p>{groups.map((group) => <div key={group.label} className="shrink-0 lg:w-full"><p className="mb-1 px-1 text-[0.65rem] font-extrabold uppercase tracking-[0.14em] text-ink-faint">{group.label}</p><div className="flex gap-1 lg:flex-col">{group.links.map(([id, label]) => <a key={id} href={id === "peta-rumah" ? "/admin/peta-rumah" : id === "kas" ? "/admin/kas" : `#${id}`} className={`shrink-0 rounded-lg px-3 py-2.5 text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand ${id === "antrean" ? "bg-brand-soft text-brand-deep" : "text-ink-muted hover:bg-surface-subtle hover:text-ink"}`}>{label}</a>)}</div></div>)}</nav>;
}
...
function AdminActivityNotifications({ activities }: { activities: AdminActivityRow[] }) {
  return <details className="group relative"><summary aria-label="Buka notifikasi aktivitas operasional" className="relative grid size-11 cursor-pointer list-none place-items-center rounded-full border border-line bg-surface text-ink transition hover:-translate-y-0.5 hover:border-brand hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 [&::-webkit-details-marker]:hidden"><Bell size={20} weight="fill" aria-hidden="true" />{activities.length ? <span className="absolute -right-1 -top-1 grid min-w-5 place-items-center rounded-full bg-brand px-1 text-[10px] font-extrabold leading-5 text-on-brand">{activities.length > 9 ? "9+" : activities.length}</span> : null}</summary><section aria-label="Notifikasi aktivitas operasional" className="absolute right-0 z-30 mt-3 w-[min(24rem,calc(100vw-2.5rem))] overflow-hidden rounded-[1.25rem] border border-line bg-surface-raised shadow-[0_22px_60px_rgba(4,28,24,0.2)]"><div className="flex items-start justify-between gap-4 border-b border-line px-5 py-4"><div><p className="text-sm font-extrabold text-ink">Aktivitas terbaru</p><p className="mt-1 text-xs leading-5 text-ink-muted">Kronologi operasional tanpa KTP, KK, atau isi pengajuan.</p></div><span className="shrink-0 text-xs font-extrabold text-brand-deep">{activities.length} log</span></div>{activities.length ? <div className="max-h-[min(28rem,calc(100dvh-10rem))] overflow-y-auto">{activities.map((activity) => <article key={activity.id} className="flex gap-3 border-b border-line px-5 py-4 last:border-b-0"><span className="mt-1.5 size-2 shrink-0 rounded-full bg-brand" aria-hidden="true" /><div className="min-w-0"><p className="text-sm font-extrabold leading-5 text-ink">{activity.action}</p><p className="mt-1 text-xs leading-5 text-ink-muted">{activity.actor_email === "Warga" ? "Warga OPAL" : activity.actor_email} · <time dateTime={activity.created_at}>{formatAdminDateTime(activity.created_at)}</time></p></div></article>)}</div> : <p className="px-5 py-6 text-sm leading-6 text-ink-muted">Aktivitas akan muncul saat warga mengirim data atau pengurus melakukan perubahan.</p>}</section></details>;
}

function FeeForm({ fee }: { fee: FeeRow }) {
  return <form action={saveFeeSchedule} className="rounded-[18px] border border-line bg-surface-raised p-5 sm:p-6"><h3 className="text-lg font-extrabold tracking-[-0.03em] text-ink">{fee.label}</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Nama iuran"><input name="label" defaultValue={fee.label} required className={inputClass} /></Field><Field label="Nominal (Rp)"><input name="amountRupiah" type="number" min="1" step="1" defaultValue={fee.amount_rupiah} required className={inputClass} /></Field><Field label="Berlaku mulai"><input name="effectiveFrom" type="date" defaultValue={fee.effective_from} required className={inputClass} /></Field><Field label="Metode pembayaran"><input name="paymentMethod" defaultValue={fee.payment_method} required className={inputClass} /></Field></div><Field label="Tujuan pembayaran"><input name="destination" defaultValue={fee.destination} className={inputClass} /></Field><Field label="Keterangan"><textarea name="description" defaultValue={fee.description} className={textareaClass} /></Field><button className="mt-5 inline-flex min-h-11 items-center justify-center rounded-full bg-action px-5 text-sm font-bold text-on-action transition hover:-translate-y-0.5 hover:bg-brand hover:text-on-brand">Simpan jadwal baru</button></form>;
}

function AnnouncementForm({ announcement }: { announcement?: AnnouncementRow }) {
  return <form action={saveAnnouncement} className="rounded-[18px] border border-line bg-surface-raised p-5 sm:p-6"><input type="hidden" name="id" value={announcement?.id ?? ""} /><h3 className="text-lg font-extrabold tracking-[-0.03em] text-ink">{announcement ? "Ubah pengumuman" : "Buat pengumuman"}</h3><div className="mt-5 grid gap-4"><Field label="Judul"><input name="title" defaultValue={announcement?.title} required className={inputClass} /></Field><Field label="Isi"><textarea name="body" defaultValue={announcement?.body} required className={textareaClass} /></Field><Field label="Tanggal publikasi"><input name="publishedAt" type="date" defaultValue={announcement?.published_at ?? currentJakartaDate()} required className={inputClass} /></Field><div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(12rem,0.7fr)]"><Field label={announcement?.image_path ? "Ganti gambar (opsional)" : "Gambar informasi (opsional)"}><input name="image" type="file" accept="image/jpeg,image/png,image/webp" className="mt-2 block min-h-11 w-full cursor-pointer rounded-xl border border-dashed border-line bg-surface px-3 py-2.5 text-sm text-ink-muted file:mr-3 file:rounded-lg file:border-0 file:bg-brand-soft file:px-3 file:py-2 file:font-extrabold file:text-brand-deep hover:border-brand" /><span className="mt-1.5 block text-xs font-semibold leading-5 text-ink-faint">JPG, PNG, atau WEBP. Maksimal 5 MB.</span></Field><Field label="Teks alternatif"><input name="imageAlt" defaultValue={announcement?.image_alt ?? ""} placeholder="Contoh: Warga kerja bakti" maxLength={200} className={inputClass} /><span className="mt-1.5 block text-xs font-semibold leading-5 text-ink-faint">Dipakai pembaca layar; jika kosong, judul digunakan.</span></Field></div>{announcement?.image_path ? <div className="flex flex-wrap items-center gap-4 rounded-xl border border-line bg-surface-subtle p-3"><Image src={`/api/announcement-image/${announcement.id}`} alt={announcement.image_alt || announcement.title} width={80} height={80} unoptimized className="size-20 rounded-lg object-cover" /><div className="min-w-0 flex-1"><p className="text-sm font-extrabold text-ink">Gambar saat ini tersimpan</p><p className="mt-1 truncate text-xs font-semibold text-ink-muted">Unggah gambar baru untuk menggantinya.</p></div><Toggle name="removeImage" label="Hapus gambar" checked={false} /></div> : null}<div className="flex flex-wrap gap-x-5 gap-y-2"><Toggle name="pinned" label="Sematkan di beranda" checked={announcement?.pinned ?? false} /><Toggle name="published" label="Tampilkan ke warga" checked={announcement?.published ?? true} /></div></div><div className="mt-5 flex flex-wrap items-center gap-3"><button type="submit" className="inline-flex min-h-11 items-center justify-center rounded-full bg-action px-5 text-sm font-bold text-on-action transition hover:-translate-y-0.5 hover:bg-brand hover:text-on-brand">Simpan pengumuman</button>{announcement ? <><button type="submit" formAction={deleteAnnouncement} className="inline-flex min-h-11 items-center justify-center rounded-full border border-danger/35 px-4 text-sm font-bold text-danger transition hover:border-danger hover:bg-danger-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger">Hapus pengumuman</button><p className="basis-full text-xs leading-5 text-ink-faint">Menghapus pengumuman juga menghapus gambar yang tersimpan.</p></> : null}</div></form>;
}

function ResourceForm({ resource }: { resource?: ResourceRow }) {
  return <form action={saveResource} className="rounded-[18px] border border-line bg-surface-raised p-5 sm:p-6"><input type="hidden" name="id" value={resource?.id ?? ""} /><h3 className="text-lg font-extrabold tracking-[-0.03em] text-ink">{resource ? resource.title : "Tambah layanan"}</h3><div className="mt-5 grid gap-4"><Field label="Judul"><input name="title" defaultValue={resource?.title} required className={inputClass} /></Field><Field label="Deskripsi"><textarea name="description" defaultValue={resource?.description} required className={textareaClass} /></Field><Field label="Tautan sumber"><input name="href" type="url" defaultValue={resource?.href} required className={inputClass} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Kategori"><select name="category" defaultValue={resource?.category ?? "Surat"} className={inputClass}><option>Keuangan</option><option>Surat</option><option>Data warga</option><option>Fasilitas</option><option>Rumah</option></select></Field><Field label="Urutan"><input name="sortOrder" type="number" min="1" step="1" defaultValue={resource?.sort_order ?? 99} required className={inputClass} /></Field></div><div className="flex flex-wrap gap-x-5 gap-y-2"><Toggle name="requiresGoogleLogin" label="Perlu login Google" checked={resource?.requires_google_login ?? false} /><Toggle name="published" label="Tampilkan ke warga" checked={resource?.published ?? true} /></div></div><button className="mt-5 inline-flex min-h-11 items-center justify-center rounded-full bg-action px-5 text-sm font-bold text-on-action transition hover:-translate-y-0.5 hover:bg-brand hover:text-on-brand">Simpan layanan</button></form>;
}

function GuideForm({ section }: { section: GuideRow }) {
  return <form action={saveGuideSection} className="rounded-[18px] border border-line bg-surface-raised p-5 sm:p-6"><input type="hidden" name="id" value={section.id} /><div className="grid gap-4 md:grid-cols-[1fr_8rem]"><Field label="Judul"><input name="title" defaultValue={section.title} required className={inputClass} /></Field><Field label="Urutan"><input name="sortOrder" type="number" min="1" step="1" defaultValue={section.sort_order} required className={inputClass} /></Field></div><Field label="Ringkasan"><textarea name="summary" defaultValue={section.summary} required className={textareaClass} /></Field><div className="mt-4"><MarkdownEditor name="bodyMarkdown" label="Isi panduan (Markdown)" defaultValue={section.body_markdown} /></div><div className="mt-5 flex flex-wrap items-center justify-between gap-4"><Toggle name="published" label="Tampilkan ke warga" checked={section.published} /><button className="inline-flex min-h-11 items-center justify-center rounded-full bg-action px-5 text-sm font-bold text-on-action transition hover:-translate-y-0.5 hover:bg-brand hover:text-on-brand">Simpan panduan</button></div></form>;
}
...
function CashSection({ cashBalance, income, expense, transactionCount, lastUpdated, transactions }: { cashBalance: number; income: number; expense: number; transactionCount: number; lastUpdated: string | null; transactions: CashTransactionRow[] }) {
  return <div className="space-y-6"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-line bg-surface-raised p-4"><p className="text-xs font-extrabold uppercase tracking-[0.12em] text-ink-faint">Saldo berjalan</p><p className="mt-2 text-2xl font-extrabold tracking-[-0.06em] text-ink">{formatRupiah(cashBalance)}</p></div><div className="rounded-2xl border border-line bg-surface-raised p-4"><p className="text-xs font-extrabold uppercase tracking-[0.12em] text-ink-faint">Total masuk</p><p className="mt-2 text-2xl font-extrabold tracking-[-0.06em] text-brand-deep">{formatRupiah(income)}</p></div><div className="rounded-2xl border border-line bg-surface-raised p-4"><p className="text-xs font-extrabold uppercase tracking-[0.12em] text-ink-faint">Total keluar</p><p className="mt-2 text-2xl font-extrabold tracking-[-0.06em] text-danger-deep">{formatRupiah(expense)}</p></div></div><div className="flex flex-col gap-3 border-l-2 border-brand bg-brand-soft/60 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5"><div><p className="text-sm font-extrabold text-brand-deep">Sumber data Kas sudah native di database</p><p className="mt-1 text-sm leading-6 text-ink-muted">{transactionCount} transaksi tersedia untuk pengurus. {lastUpdated ? `Terakhir diperbarui ${formatAdminDate(lastUpdated)}.` : "Belum ada transaksi."}</p></div><a href="/admin/kas" className="inline-flex min-h-10 shrink-0 items-center justify-center rounded-xl bg-action px-4 text-sm font-extrabold text-on-action transition hover:bg-brand hover:text-on-brand">Buka ruang kerja Kas</a></div><div className="grid gap-8 xl:grid-cols-[minmax(20rem,0.72fr)_minmax(0,1.28fr)]"><AdminCashTransactionEditor /><CashTransactionLedger transactions={transactions} /></div></div>;
}

function CashTransactionLedger({ transactions }: { transactions: CashTransactionRow[] }) {
    return <section aria-labelledby="cash-ledger-heading" className="border-t border-line pt-5 xl:border-t-0 xl:border-l xl:pl-8"><div className="flex items-baseline justify-between gap-4"><div><h3 id="cash-ledger-heading" className="text-lg font-extrabold tracking-[-0.035em] text-ink">Transaksi terbaru</h3><p className="mt-1 text-sm leading-6 text-ink-muted">Buka satu baris untuk memperbaiki catatannya.</p></div><a href="/admin/kas" className="text-sm font-extrabold text-brand-deep hover:text-brand">Lihat semua {transactions.length}</a></div>{transactions.length ? <div className="mt-4 border-t border-line">{transactions.slice(0, 12).map((transaction) => <article key={transaction.id} className="border-b border-line py-4 last:border-b-0"><details><summary className="grid cursor-pointer list-none gap-3 [&::-webkit-details-marker]:hidden sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><p className="truncate font-extrabold text-ink">{transaction.category}</p><p className="mt-1 truncate text-sm text-ink-muted">{formatAdminDate(transaction.transaction_date)}{transaction.description ? `, ${transaction.description}` : ""}</p></div><div className="flex items-center justify-between gap-3 sm:justify-end"><span className={`inline-flex shrink-0 items-center rounded-lg px-2.5 py-1.5 text-xs font-extrabold leading-none ${transaction.direction === "income" ? "bg-brand-soft text-brand-deep" : "bg-danger-soft text-danger-deep"}`}>{transaction.direction === "income" ? "Masuk" : "Keluar"}</span><p className="min-w-[7.5rem] text-right text-base font-extrabold tracking-[-0.03em] text-ink sm:text-lg">{formatRupiah(transaction.amount_rupiah)}</p></div></summary><div className="mt-5"><AdminCashTransactionEditor transaction={transaction} /></div></details></article>)}</div> : <EmptyQueue label="Belum ada transaksi Kas yang dicatat." />}</section>;
}

function AdminUsersPanel({ users, currentEmail }: { users: AdminUserRow[]; currentEmail: string }) {
  return <div className="grid gap-8 xl:grid-cols-[minmax(0,0.9fr)_minmax(20rem,1.1fr)]"><form action={addAdminUser} className="rounded-[18px] border border-line bg-surface-raised p-5 sm:p-6"><h3 className="text-lg font-extrabold tracking-[-0.035em] text-ink">Tambah pengurus</h3><p className="mt-1 text-sm leading-6 text-ink-muted">Masukkan email yang akan diberi akses. Pengurus tersebut juga harus memiliki akun email/password di Supabase Authentication.</p><Field label="Email pengurus"><input name="email" type="email" autoComplete="email" placeholder="pengurus@contoh.com" required className={inputClass} /></Field><button className="mt-5 inline-flex min-h-11 rounded-full bg-action px-5 text-sm font-bold text-on-action transition hover:bg-brand hover:text-on-brand">Berikan akses admin</button></form><section aria-labelledby="admin-users-heading" className="border-t border-line pt-5 xl:border-t-0 xl:border-l xl:pl-8"><div className="flex items-baseline justify-between gap-3"><div><h3 id="admin-users-heading" className="text-lg font-extrabold tracking-[-0.035em] text-ink">Pengurus berakses</h3><p className="mt-1 text-sm leading-6 text-ink-muted">{users.length} email dapat masuk ke ruang kerja RT.</p></div></div><div className="mt-4 border-t border-line">{users.map((user) => { const isCurrent = user.email === currentEmail; return <article key={user.email} className="flex flex-wrap items-center justify-between gap-3 border-b border-line py-4"><div className="min-w-0"><p className="break-all font-extrabold text-ink">{user.email}</p><p className="mt-1 text-sm text-ink-muted">Ditambahkan {formatAdminDate(user.created_at)}</p></div>{isCurrent ? <span className="rounded-full bg-brand-soft px-2.5 py-1 text-xs font-extrabold text-brand-deep">Akun Anda</span> : <form action={removeAdminUser}><input type="hidden" name="email" value={user.email} /><button className="min-h-10 rounded-full border border-line px-3.5 text-xs font-bold text-ink transition hover:border-danger hover:text-danger">Cabut akses</button></form>}</article>; })}</div></section></div>;
}

function StaffForm({ staff }: { staff?: StaffRow }) {
  return (
    <form action={saveStaffProfile} className="rounded-[18px] border border-line bg-surface-raised p-5">
