import { Bell, CaretDown, CheckCircle, FileLock, GearSix, HouseLine, Receipt, SignOut, Stamp, UsersThree, Wrench } from "@phosphor-icons/react/dist/ssr";
...
function AdminOverview({ pendingSubmissions, pendingRequests, pendingContributions, issuableRequests, activePropertyLinks, documentsReady, cashBalance, cashIncome, cashExpense, cashTransactionCount, lastCashTransaction }: { pendingSubmissions: number; pendingRequests: number; pendingContributions: number; issuableRequests: number; activePropertyLinks: number; documentsReady: boolean; cashBalance: number; cashIncome: number; cashExpense: number; cashTransactionCount: number; lastCashTransaction: string | null }) {
  ...
  return <section aria-labelledby="admin-overview" className="mt-8 overflow-hidden rounded-[26px] border border-line bg-surface-raised shadow-[0_18px_55px_rgba(5,45,39,0.07)]"><h2 id="admin-overview" className="sr-only">Ringkasan kerja RT</h2><div className="grid lg:grid-cols-[1.25fr_1fr]"><a href="#kas" className="group border-b border-line bg-brand-soft/40 p-6 transition-colors hover:bg-brand-soft sm:p-8 lg:border-b-0 lg:border-r"><div className="flex items-start justify-between gap-5"><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-brand-deep">Saldo Kas tercatat</p><p className="mt-4 text-4xl font-extrabold tracking-[-0.08em] text-ink sm:text-5xl">{formatRupiah(cashBalance)}</p></div><span className="grid size-11 place-items-center rounded-xl bg-brand text-on-brand"><Receipt size={22} weight="fill" aria-hidden="true" /></span></div><div className="mt-7 grid grid-cols-2 gap-5 border-t border-brand/20 pt-5"><div><p className="text-xs font-bold text-ink-muted">Pemasukan</p><p className="mt-1 text-lg font-extrabold tracking-[-0.04em] text-brand-deep">{formatRupiah(cashIncome)}</p></div><div><p className="text-xs font-bold text-ink-muted">Pengeluaran</p><p className="mt-1 text-lg font-extrabold tracking-[-0.04em] text-danger-deep">{formatRupiah(cashExpense)}</p></div></div><p className="mt-5 text-sm leading-6 text-ink-muted">{cashTransactionCount} transaksi tersimpan{lastCashTransaction ? `, terakhir ${formatAdminDate(lastCashTransaction)}.` : "."}</p><span className="mt-5 inline-flex text-sm font-extrabold text-brand-deep group-hover:text-brand">Buka ruang kerja Kas →</span></a><div className="grid sm:grid-cols-2"><a href="#antrean" className="group border-b border-line p-6 transition-colors hover:bg-surface-subtle sm:p-7 sm:border-r"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">Tindakan hari ini</p><p className="mt-3 text-4xl font-extrabold tracking-[-0.08em] text-ink">{pendingTotal}</p><p className="mt-2 text-sm leading-6 text-ink-muted">{pendingSubmissions} pendataan, {pendingRequests} surat, {pendingContributions} iuran perlu dicek.</p><span className="mt-4 inline-flex text-sm font-extrabold text-brand-deep group-hover:text-brand">Buka antrean →</span></a><a href="#surat" className="group border-b border-line p-6 transition-colors hover:bg-surface-subtle sm:p-7"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">Surat siap terbit</p><p className="mt-3 text-4xl font-extrabold tracking-[-0.08em] text-ink">{issuableRequests}</p><p className="mt-2 text-sm leading-6 text-ink-muted">{issuableRequests ? "Sudah disetujui dan menunggu PDF." : "Belum ada yang menunggu penerbitan."}</p><span className="mt-4 inline-flex text-sm font-extrabold text-brand-deep group-hover:text-brand">Kelola surat →</span></a><a href="#surat" className="group p-6 transition-colors hover:bg-surface-subtle sm:p-7 sm:border-r"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">Penerbitan surat</p><p className="mt-3 text-lg font-extrabold tracking-[-0.04em] text-ink">{documentsReady ? "Siap digunakan" : "Perlu dilengkapi"}</p><p className="mt-2 text-sm leading-6 text-ink-muted">{documentsReady ? "Identitas RT lengkap." : "Lengkapi identitas penerbit."}</p></a><a href="#rumah" className="group p-6 transition-colors hover:bg-surface-subtle sm:p-7"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">Akses rumah aktif</p><p className="mt-3 text-4xl font-extrabold tracking-[-0.08em] text-ink">{activePropertyLinks}</p><p className="mt-2 text-sm leading-6 text-ink-muted">Tautan privat yang masih dapat dipakai warga.</p></a></div></div></section>;
}

function AdminNavigation() {
  const links = [["antrean", "Antrean"], ["surat", "Surat"], ["arsip-surat", "Arsip surat"], ["kas", "Kas"], ["peta-rumah", "Peta Rumah"], ["rumah", "Rumah"], ["iuran", "Iuran aktif"], ["iuran-rumah", "Iuran rumah"], ["pengumuman", "Pengumuman"], ["layanan", "Layanan"], ["panduan", "Panduan"], ["fasilitas", "Fasilitas"], ["pengurus", "Pengurus"]] as const;
  return <nav aria-label="Bagian admin" className="-mx-5 flex gap-1 overflow-x-auto border-y border-line bg-surface px-5 py-2 sm:mx-0 sm:px-0 lg:sticky lg:top-24 lg:mx-0 lg:h-fit lg:flex-col lg:gap-1 lg:overflow-visible lg:border-0 lg:bg-transparent lg:px-0 lg:py-0"><p className="sr-only lg:not-sr-only lg:mb-2 lg:px-3 lg:text-xs lg:font-extrabold lg:uppercase lg:tracking-[0.14em] lg:text-ink-faint">Navigasi ruang kerja</p>{links.map(([id, label]) => <a key={id} href={id === "peta-rumah" ? "/admin/peta-rumah" : id === "kas" ? "/admin/kas" : `#${id}`} className={`shrink-0 rounded-xl px-3.5 py-2.5 text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand ${id === "kas" ? "bg-brand-soft text-brand-deep" : "text-ink-muted hover:bg-surface-subtle hover:text-ink"}`}>{label}</a>)}</nav>;
}
