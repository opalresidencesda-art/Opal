...
import { AdminCashRowsLoadingLayer, AdminCashFilterBoundary } from "@/components/admin-cash-filter-boundary";
import { AdminCashTransactionEditor, type AdminCashTransaction } from "@/components/admin-cash-transaction-editor";
import { AdminKasMonthlyDues } from "@/components/admin-kas-monthly-dues";
import { getAdminContext } from "@/lib/admin";
import { getAllCashTransactions } from "@/lib/cash";
import { formatRupiah } from "@/lib/format";
import { buildMonthlyDuesRows, jakartaPeriod, KAS_OPAL_CONTRIBUTION_CATEGORY, isPeriodMonth, summarizeMonthlyDues, type ContributionStatus } from "@/lib/monthly-dues";
...
type DuesPropertyRow = { id: string; unit_code: string; resident_profiles: { responsible_name: string } | { responsible_name: string }[] | null };
type DuesContributionRow = { id: string; property_id: string; period: string | null; amount_rupiah: number; paid_at: string | null; status: ContributionStatus };
type KasFeeRow = { amount_rupiah: number };
...
export default async function AdminKasPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const currentDuesPeriod = jakartaPeriod();
  const suppliedDuesPeriod = toText(params.iuranPeriod);
  const iuranPeriod = isPeriodMonth(suppliedDuesPeriod) ? suppliedDuesPeriod : currentDuesPeriod;
  const suppliedDuesView = toText(params.iuranView);
  const iuranView: DuesView = ["all", "attention", "paid", "unprepared"].includes(suppliedDuesView) ? suppliedDuesView as DuesView : "all";
  const [result, duesPropertiesResult, duesContributionsResult, kasFeeResult] = await Promise.all([
    getAllCashTransactions(supabase),
    supabase.from("properties").select("id,unit_code,resident_profiles(responsible_name)").eq("active", true).order("unit_code"),
    supabase.from("property_contributions").select("id,property_id,period,amount_rupiah,paid_at,status").eq("category", KAS_OPAL_CONTRIBUTION_CATEGORY).order("period", { ascending: false }),
  ...
  const duesLoadError = Boolean(duesPropertiesResult.error || duesContributionsResult.error || kasFeeResult.error);
  const duesProperties = ((duesPropertiesResult.data ?? []) as DuesPropertyRow[]).map((property) => {
    const profile = Array.isArray(property.resident_profiles) ? property.resident_profiles[0] : property.resident_profiles;
    return { id: property.id, unitCode: property.unit_code, responsibleName: profile?.responsible_name ?? null };
  });
  const duesContributions = (duesContributionsResult.data ?? []) as DuesContributionRow[];
  const duesRows = buildMonthlyDuesRows(duesProperties, duesContributions.map((contribution) => ({
    id: contribution.id,
    propertyId: contribution.property_id,
    period: contribution.period,
    amountRupiah: contribution.amount_rupiah,
    paidAt: contribution.paid_at,
    status: contribution.status,
  })), iuranPeriod);
  const duesSummary = summarizeMonthlyDues(duesRows);
  const activeFeeAmount = (kasFeeResult.data as KasFeeRow | null)?.amount_rupiah ?? null;
  ...
  const pageParams = { ...filters, iuranPeriod, iuranView };
  const cashReturnTo = `/admin/kas?iuranPeriod=${encodeURIComponent(iuranPeriod)}&iuranView=${iuranView}`;
  ...
  return <main className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10 lg:py-12">
    <div className="flex flex-col gap-5 border-b border-line pb-7 sm:flex-row sm:items-end sm:justify-between">
      <div><Link href="/admin" className="text-sm font-extrabold text-brand-deep hover:text-brand">← Kembali ke ruang kerja RT</Link><div className="mt-6 flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-brand-soft text-brand-deep"><Receipt size={23} weight="fill" aria-hidden="true" /></span><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-brand-deep">Buku Kas OPAL</p><h1 className="mt-1 text-3xl font-extrabold tracking-[-0.065em] text-ink sm:text-4xl">Ruang kerja uang masuk dan keluar.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-ink-muted">Satu sumber data untuk bendahara. Pantau status Iuran Kas OPAL per rumah tanpa mencampurkannya dengan transaksi uang yang benar-benar diterima.</p></div></div></div>
